...
- [X] T011 [P] [US1] Contract test: tests/contract/test_card_create_contract.spec.ts (validate POST /api/boards/:boardId/columns/:columnId/cards response shape)
...
- [X] T013 [P] [US1] Implement `POST /api/boards/:boardId/columns/:columnId/cards` handler in backend/src/controllers/cards.controller.ts
...
- [X] T025 [P] [US3] Contract test: tests/contract/test_card_delete_contract.spec.ts (validate DELETE /api/cards/:cardId)

- [X] T026 [P] [US3] Integration test: tests/integration/test_delete_card.spec.ts (delete card end-to-end, verify DB and realtime event)

Implementation

- [X] T027 [US3] Implement DELETE /api/cards/:cardId handler in backend/src/controllers/cards.controller.ts

- [X] T028 [P] [US3] Implement backend delete logic in backend/src/services/cardService.ts

- [X] T029 [P] [US3] Emit realtime card:deleted event with { cardId, boardId, columnId } in backend/src/realtime/index.ts

- [X] T030 [P] [US3] Add frontend delete UI and confirmation component in frontend/src/features/card/DeleteCard.tsx and ensure Card detail view closes when deleted

- [X] T030A [P] [US3] Handle realtime card:deleted events in board and card detail views

- [X] T030B [P] [US3] Add frontend tests validating card detail view closes when card is deleted by another user

- [X] T030C [P] [US3] Add realtime synchronization tests for delete-event propagation
...
- [X] T036 Validate card detail viewing functionality (FR-003)

- [X] T037 Verify users can open and view card details from the board

- [X] T038 Validate SC-004 by testing create, edit, and delete workflows and confirming at least 90% of users complete these actions successfully on their first attempt
...
- [X] T040 Validate title field rules (required, minimum 1 character, maximum 100 characters) across frontend and backend validation
- [X] T041 Add Playwright end-to-end tests for create, view detail, edit, and delete Card workflows in tests/e2e/card-crud.spec.ts
---
